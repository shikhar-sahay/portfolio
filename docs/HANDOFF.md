...
**Milestone:** v4.3 eight-issue polish pass (hero letters, hero exit, statements, timeline, skills marks, carousel, control panel, footer)
**Status:** IMPLEMENTED and browser-verified on production build; awaiting owner review
...
- **Hero (Identity):** editorial arch aperture portrait (rounded crown, offset vermilion echo arch, pointer parallax), display type overlapping the arch, statement, compact context row; the name reacts letter by letter under the pointer (`InteractiveLetters`: 2D gaussian rise field with tight vertical falloff so stacked rows stay independent, per-letter springs, scroll-aware center caching, fine pointers only); rows separated with explicit overflow-visible boundaries; scene-change scroll transition compressed to 120svh with every layer holding the frame until late (no dead scroll); reduced motion never pins the scene
- **Statements bridge:** three oversized statements in full-width registers (flush-left BUILD, flush-right larger BREAK, indented REBUILD finale) sharing one sticky ink stage from the first pixel; scroll moves emphasis down the stack (inactive lines recede to 0.16 opacity and 0.94 scale); rows interlock with controlled overlap; lateral drift throughout; 200svh; releases directly into About with no blank screens
...
- **Skills:** three drifting marquee rows of golden emblems (alternate directions, pause on hover, edge fade); no counts; real CC0 brand marks for 19 of 23 tools (C, C++, R added in v4.3; SQL, Nmap, CrowdStrike Falcon, Beelzebub keep honest monograms since no genuine mark exists); certifications as their own hairline register
...
- **Control center:** framed instrument panel with accent corner ticks, title strip (live status dot), six hairline-divided modules (Now, IST clock, Studying, Toolbox, Channels, Navigate), footer strip (Vellore, India · UTC +05:30, session uptime)
- **Footer:** compact contact grid (Let's talk gesture + Email, Elsewhere, Pages columns; server placeholders are plain text, never anchors), slim resume row (Everything, condensed.), closing marquee wordmark (SHIKHAR SAHAY repeating, letters reactive with vertical bleed inside the mask, pauses on hover, static fitted wordmark under reduced motion), designed-and-built meta
...
- Brand marks for SQL, Nmap, CrowdStrike Falcon, Beelzebub (no genuine CC0 mark found; monograms stand in)
...
| First Load JS ~153 kB, budget 150 kB                                        | Medium   | +3 kB over the 150 line (was ~151 at v4.2). Reclaim by replacing the InteractiveLetters accent tint with a cheaper effect, trimming carousel hint state, or moving Counter out of the lazy chunk |
...
| build                    | Pass      | Static; First Load JS ~153 kB (budget 150, see KNOWN ISSUES)                                                                                                                                                      |
| Browser validation       | Performed | Production build, DOM/bbox/scroll-state harnesses (no screenshot loops): hero letters 11/11, hero exit 5/5, statements 10/10, timeline 14/14, skills 10/10, carousel 19/19, control+footer 26/26, final e2e 12/12 |
...
| Favicon                  | Added     | `src/app/icon.svg` (ink field, vermilion diamond); no more /favicon.ico 404                                                                                                                                       |
