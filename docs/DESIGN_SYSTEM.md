...
- Hero scene `h-[120svh]` with `h-dvh` sticky; statements use `h-[120svh] sm:h-[200svh]` pulled up `-mt-[100dvh]`, with compact stack travel from `0vh` to `-18vh` so the mobile handoff starts onstage while desktop keeps the longer composition
...
- **Pieces fragments:** Pieces of Me has four fragments: Literature, Music, Football, Side Quests. The selector is a clean four-item instrument (two columns below lg), with selected state and keyboard behavior preserved. The Notes Wall manifesto below carries the people and community idea, so no fifth fragment is needed.
