...
### 68. Pieces Content plus Image Delivery: Blog Metadata, Slogan Removal, Favourites, Derivative Photo (EXPERIMENTAL polish)

**Decision:** Content and delivery pass over #67, verified with browser-measured before and after numbers. No API, matcher, persistence, dependency, or Motion changes.

1. Literature sheets now read TECHNOLOGY · BLOG on both articles (same centered dot, same small-caps treatment). The final paragraph ends with the specified tech blogs sentence (emphasis underline preserved on the opening sentence only). Desktop alignment is exact: prose bottom equals archives grid bottom at 1440 and 1280 (0px both, was 15px below).
2. Football slogan deleted entirely (no replacement, field removed from content and render). The prose now ends at the Camp Nou sentence. Photo widened one step (520px, 540px xl) to meet the shorter column; centered composition terminates within 31px at 1440 and 26px at 1280, balanced by eye.
3. Football image quality root cause: the default AVIF path. Forensics on the production build showed the 500px slot correctly selecting 640w at DPR1 and 1080w at DPR2 with q=90 in the URL, but the delivered AVIF measured 32 KB (640px) and 57 KB (1080px). A 1:1 contact sheet (ideal Lanczos downscale vs q90 AVIF vs q100 AVIF vs q90 WebP) showed visible smoothing on cap lettering, crest, and shirt in q90 AVIF, while WebP q90 and a prepared 1200px JPEG derivative matched the ideal. Fix: checked-in `football-barca-1200.jpg` (1200x800, q90 progressive, 175 KB) served via Next/Image `unoptimized` with the same 4:3 CSS crop. No global config change, so hero, projects, and Spotify artwork delivery are untouched. Post-fix verification: content-type image/jpeg, 174,917 bytes, natural 1200x800, transform none, headless verdict crisp with readable cap lettering and sharp crest at both DPRs. The original 7.4 MB JPEG stays in the repo as the derivative source; the `.JPG` ambient declaration stays for it.
4. Side Quests prose drops the parentheses and the serif italic aside (plain "concerning", zero em/i nodes). Eyebrow is now FOR THE WATCHLIST. WHAT USUALLY WORKS is deleted everywhere. MY FAVOURITES follows the helper line behind a thin rule: five static ranked rows (accent 01 to 05, uppercase titles) in the locked order SUITS, PRISON BREAK, MONEY HEIST, PEAKY BLINDERS, THE MENTALIST. No links, ratings, images, or motion. Natural flow replaces the old bottom pinning, so the idle void is gone (helper to separator 76px, live region to separator 20px). Baselines: card ends 20px below prose at 1440 and 29px at 1280 (was 76px and 85px over with the void).
5. Music first paragraph takes the finalized painfully white-washed sentence verbatim; second paragraph untouched. The longer paragraph moves the 1440 baseline from 13px to 7px and 1280 to 16px; the structural stretch absorbs it with no further edits. Mobile and tablet wrap naturally.
6. Spacing audit (measurement only, nothing implemented): sections are edge-adjacent everywhere; all rhythm is section padding plus internal margins. Tallest systematic gap is Skills emblem rows to Certifications eyebrow (334px at 1440, triple stack of closing margin plus Toolkit padding plus Certifications padding). Stacked widths inflate vh-based gaps in absolute px (820 readings). Tightest systematic rhythm is the 32px eyebrow to heading step. Full table and classifications recorded in HANDOFF; all recommendations pending owner decision.

**Status:** EXPERIMENTAL

**Date:** 2026-09-11

**Rationale:** Metadata should name the actual body of work (blogs, not essays). A lookup slogan is not a closing; the prose ends stronger without it. Favourites say more about taste than generic preference copy and fill the artifact with meaning instead of pinned air. The optimizer cannot be tuned per asset for format, so a deterministic derivative is the clean local fix that cannot regress other images.
...
1. Every route now receives a Content Security Policy, `frame-ancestors 'none'`, `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, a deny-by-default Permissions-Policy for sensitive device features, and HSTS for the HTTPS-only Vercel deployment. The CSP is source-specific: self scripts plus inline scripts required by Next's production bootstrap and RSC payloads, self images plus data/blob and Spotify artwork, self connections, self fonts, inline styles for framework style injection, and no object or media embeds.
