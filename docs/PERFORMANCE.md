...
## v4.9 Status (2026-09-07)

- First Load JS: ~160 kB against the 150 kB budget (was ~159 kB at v4.8). The delta covers the Certifications client section, the wall Latest flight, and the footer rewrite (the velocity-wave deletion offsets part of it).
- New runtime costs are event-gated only: tagline springs, footer lean loop (pointer-inside plus settle, zero layout reads), Latest flight rAF (650ms, then stops), placement ghost (direct DOM writes). No new idle loops.
- Reclamation options (unchanged): cheapen the InteractiveLetters tint, trim carousel hint state, or move Counter out of the lazy chunk. Track in HANDOFF known issues.
