...
- Below sm, and on short landscape viewports up to 1023px wide, a compact cut of the same choreography runs: 155svh section, tighter stack travel, lower finale resolve, quieter inactive rows, and a nearly immediate bridge opacity gate. The gate prevents the overlapped bridge from leaking BREAK or REBUILD into the landing viewport before the hero releases. Desktop values are untouched.
