...
const lines = [
  { text: 'I build.', note: 'tools, experiments, platforms', indent: '0vw', drift: [-1.5, -3.5] },
  {
    text: 'I break.',
  ...
  { text: 'I rebuild.', note: 'better than before', indent: '18vw', drift: [5, 1.5] },
] as const;
...
function StatementLine({
  ...
  // stack. The stage is never empty. Each line also drifts laterally for
  // its whole life so the composition keeps moving.
  ...
  const opacity = useTransform(
    ...
    index === 0 ? [1, 1, on, on] : isLast ? [on, 1, 1] : [on, 1, 1, on]
  );
  const y = useTransform(
    progress,
    index === 0 ? [0, 0.5, 1] : [activate - FADE, activate + FADE, 1],
    index === 0 ? [0, 0, 0] : [36, 0, 0]
  );
  ...
  return (
    <motion.div
      style={{ opacity, y, x, paddingLeft: line.indent }}
 ...
 * own the viewport together. Emphasis moves down the stack with scroll
 * (the inactive lines stay visible but receded), so there is no dead
 * stage time, and every line drifts laterally so the frame keeps moving.
